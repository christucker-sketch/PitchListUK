...
export class UkControllerStateDurableObject extends DurableObject {
  constructor(ctx, env) {
    super(ctx, env);
    this.ctx.storage.sql.exec(`
      CREATE TABLE IF NOT EXISTS uk_controller_state (
  ...
  async write(raw, { source, authority }) {
    ...
    if (parsed.controller_kind !== 'uk') throw new Error('uk_controller_snapshot_kind_invalid');
    if (!['shadow', 'authoritative'].includes(authority)) throw new Error('uk_controller_authority_invalid');
    const previous = this.readRow();
    const version = Number(previous?.version || 0) + 1;
    const sha256 = await sha256Hex(raw);
    const importedAt = new Date().toISOString();
    this.ctx.storage.sql.exec(`
      INSERT INTO uk_controller_state (singleton, version, sha256, payload, imported_at, source, authority)
      VALUES (1, ?, ?, ?, ?, ?, ?)
  ...
  precondition(request, row) {
    ...
    if (!Number.isInteger(version) || version <= 0 || !validSha256(sha256)) return { error: 'uk_controller_checkpoint_precondition_invalid', status: 400 };
    if (Number(row.version) !== version || String(row.sha256).toLowerCase() !== sha256) return { error: 'uk_controller_state_precondition_failed', status: 409 };
    return null;
  }

  async importSnapshot(request) {
    if (this.readRow()) return Response.json({ ok: false, error: 'uk_controller_state_already_initialized' }, { status: 409 });
    const authority = request.headers.get('x-findpitches-state-authority') || 'shadow';
    const source = request.headers.get('x-findpitches-state-source') || 'cloudflare-uk-controller-bootstrap';
  ...
  async checkpoint(request) {
    ...
    if (!row) return Response.json({ ok: false, error: 'uk_controller_state_not_initialized' }, { status: 404 });
    if (row.authority !== 'authoritative') return Response.json({ ok: false, error: 'uk_controller_state_not_authoritative', current_authority: row.authority }, { status: 409 });
    ...
    const written = await this.write(await request.text(), { source: 'cloudflare-uk-controller', authority: 'authoritative' });
    return Response.json({ ok: true, changed: true, ...written }, { status: 201 });
  }
  ...
  async transitionAuthority(request, targetAuthority) {
    ...
    if (!Number.isInteger(expectedVersion) || expectedVersion <= 0 || !validSha256(expectedSha256)) {
      return Response.json({ ok: false, error: 'uk_controller_authority_precondition_invalid' }, { status: 400 });
    }
    if (Number(row.version) !== expectedVersion || String(row.sha256).toLowerCase() !== expectedSha256) {
      return Response.json({ ok: false, error: 'uk_controller_state_precondition_failed', current: { version: row.version, sha256: row.sha256, authority: row.authority } }, { status: 409 });
    }
    if (row.authority === targetAuthority) return Response.json({ ok: true, changed: false, state: { ...row, payload: undefined } });
    const expectedCurrent = targetAuthority === 'authoritative' ? 'shadow' : 'authoritative';
    if (row.authority !== expectedCurrent) return Response.json({ ok: false, error: 'uk_controller_authority_transition_invalid', current_authority: row.authority }, { status: 409 });
    const state = validateControllerStateText(row.payload);
    if (state.controller_kind !== 'uk') return Response.json({ ok: false, error: 'uk_controller_snapshot_kind_invalid' }, { status: 409 });
    if (targetAuthority === 'authoritative') {
      if (state.active_instance || state.cloud_controller_intent || state.pending_source_pr || state.pending_data_pr || state.pending_deployment) {
        return Response.json({ ok: false, error: 'uk_controller_promotion_requires_clean_checkpoint' }, { status: 409 });
      }
      if (!['ready_discovery', 'ready_acquisition'].includes(state.status)) {
        return Response.json({ ok: false, error: 'uk_controller_promotion_status_invalid', status_value: state.status }, { status: 409 });
      }
    }
    this.ctx.storage.sql.exec(`
      UPDATE uk_controller_state
      SET authority = ?, source = ?
      WHERE singleton = 1 AND version = ? AND sha256 = ? AND authority = ?
    `, targetAuthority, targetAuthority === 'authoritative' ? 'cloudflare-uk-controller-cutover' : 'cloudflare-uk-controller-rollback', expectedVersion, expectedSha256, expectedCurrent);
    const updated = this.readRow();
    if (!updated || updated.authority !== targetAuthority) return Response.json({ ok: false, error: 'uk_controller_authority_transition_failed' }, { status: 409 });
  ...
  async fetch(request) {
    ...
    if (request.method === 'GET' && url.pathname === '/meta') {
      ...
      return row
        ...
        : Response.json({ ok: false, error: 'uk_controller_state_not_initialized' }, { status: 404 });
    }
