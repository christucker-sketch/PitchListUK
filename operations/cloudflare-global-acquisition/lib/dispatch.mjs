...
const CA_SOURCE_DISCOVERY_MODE = 'ca_source_discovery_pr';
const UK_PR_ONLY_MODES = Object.freeze(new Set([UK_ADDITIONS_MODE, UK_SOURCE_DISCOVERY_MODE]));
const CA_SHADOW_PR_ONLY_MODES = Object.freeze(new Set([CA_SOURCE_DISCOVERY_MODE]));
const MODES = Object.freeze({
  ...
  CA: Object.freeze(new Set([CA_SOURCE_DISCOVERY_MODE]))
});
...
export function assertGlobalControllerDispatchAllowed(env, dispatch) {
  ...
  if (level === 'pr_only' && dispatch?.mutation_capable) {
    const ukAllowed = dispatch?.country === 'UK' && UK_PR_ONLY_MODES.has(dispatch?.mode);
    const caAllowed = dispatch?.country === 'CA' && CA_SHADOW_PR_ONLY_MODES.has(dispatch?.mode) && dispatch?.shadow_only === true;
    if (!ukAllowed && !caAllowed) {
...
export {
  READ_ONLY_MODE,
  CUTOVER_READINESS_MODE,
  UK_ADDITIONS_MODE,
  UK_SOURCE_DISCOVERY_MODE,
