...
function branchName(now = new Date()) {
  return `data/cloud-texas-growth-${now.toISOString().replace(/[-:.TZ]/g, '').slice(0, 14)}`;
}
...
export class TexasAcquisitionWorkflow extends WorkflowEntrypoint {
  async run(event, step) {
    ...
    const promotion = await step.do('build controlled Texas promotion', async () => {
      return buildTexasPromotionManifest(staging, { sources: TEXAS_SOURCES });
    });
    ...
    const planned = await step.do('plan isolated Texas production delta', async () => {
      return planTexasProductionSnapshot(base.snapshot, promotion, staging, { sources: TEXAS_SOURCES });
    });
    ...
    return {
      trigger: event.schedule ? 'schedule' : 'manual',
      source_count: TEXAS_SOURCES.length,
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/health') {
      return Response.json({ ok: true, service: 'pitchlist-texas-acquisition', source_count: TEXAS_SOURCES.length });
    }
