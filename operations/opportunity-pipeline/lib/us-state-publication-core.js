...
function sameCanonicalRoutes(existing, candidate) {
  const current = identityOf(existing);
  const incoming = identityOf(candidate);
  return Boolean(current.source && current.application && current.source === incoming.source && current.application === incoming.application);
}
...
function planStateProductionSnapshot(state,snapshot,promotionManifest,stagingManifest,options={}) {
  ...
  const byId=new Map(); for(const row of existing){const i=identityOf(row);if(i.id)byId.set(i.id,row);}
  const seenIds=new Set(),seenOccurrences=new Set(),prepared=[],alreadyPresent=[];
  for(const row of candidates){
    ...
    const i=identityOf(row), incomingDate=eventDateOf(row); if(!i.source||!i.application||!i.id) throw new Error(`${scoped.name} production row missing identity evidence`);
    const idMatch=byId.get(i.id);
    if(idMatch){
      const ei=identityOf(idMatch), existingDate=eventDateOf(idMatch);
      if(ei.source===i.source&&ei.application===i.application&&(!existingDate||!incomingDate||existingDate===incomingDate)){ alreadyPresent.push({...row,id:i.id}); continue; }
      throw new Error(`${scoped.name} production identity collision:${i.id}`);
    }
    const routeMatches=existing.filter(existingRow=>{const ei=identityOf(existingRow);return ei.source===i.source||ei.application===i.application;});
    const exactOccurrence=routeMatches.find(existingRow=>sameCanonicalRouteAndDate(existingRow,row));
    if(exactOccurrence){ const ei=identityOf(exactOccurrence); alreadyPresent.push({...row,id:ei.id,stable_id:ei.id}); continue; }
    if(routeMatches.length){
      if(!incomingDate||routeMatches.some(existingRow=>!eventDateOf(existingRow)||!sameCanonicalRoutes(existingRow,row))) throw new Error(`${scoped.name} production identity collision:${i.id}`);
    }
    const occurrenceKey=incomingDate?`${i.source}\u0000${i.application}\u0000${incomingDate}`:'';
    if(seenIds.has(i.id)||(occurrenceKey&&seenOccurrences.has(occurrenceKey))) throw new Error(`${scoped.name} production duplicate:${i.id}`);
    seenIds.add(i.id); if(occurrenceKey)seenOccurrences.add(occurrenceKey); prepared.push({...row,id:i.id});
  }
