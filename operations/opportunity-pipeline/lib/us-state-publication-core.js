...
function eventDateOf(row) {
  return String(row?.event_start || row?.application_deadline || '').trim();
}
...
function sameCanonicalRoutes(existing, candidate) {
  const current = identityOf(existing);
  const incoming = identityOf(candidate);
  return Boolean(
    current.source && current.application && incoming.source && incoming.application &&
    current.source === incoming.source && current.application === incoming.application
  );
}
...
function sourceIdFromRow(row, sources = []) {
  ...
  return sources.find(item => item.source_url === sourceUrl || item.application_url === applicationUrl)?.id || '';
}
...
function assertStatePromotionInput(state, stagingManifest, sources = []) {
  ...
  if (ids.some(id => !id)) throw new Error(`${scoped.name} promotion row does not map to an approved source`);
  if (new Set(ids).size !== ids.length) throw new Error(`${scoped.name} promotion contains duplicate approved-source identities`);
  const sourceById = new Map(sources.map(source => [source.id, source]));
  for (const id of ids) {
    const source = sourceById.get(id);
    if (!source || source.status !== 'approved-pilot') throw new Error(`${scoped.name} promotion source is not approved: ${id}`);
    if (source.country_code !== 'US' || source.region_code !== scoped.code || source.jurisdiction !== scoped.jurisdiction) throw new Error(`${scoped.name} promotion source escaped ${scoped.jurisdiction} boundary: ${id}`);
  }
  for (const row of stagingManifest.rows) {
    if (row.country_code !== 'US' || row.region_code !== scoped.code || row.jurisdiction !== scoped.jurisdiction) throw new Error(`${scoped.name} promotion row escaped state boundary`);
    if (row.publishable !== false || String(row.quality_status || '').toLowerCase() !== 'review') throw new Error(`${scoped.name} promotion input row must still be review-only`);
    if (!row.stable_id || !row.event_name || !row.organiser || !row.source_url || !row.application_url) throw new Error(`${scoped.name} promotion row is missing reviewed evidence`);
  }
  return ids;
}

function heldSourceIds(stagingManifest, sources = []) {
  return [...new Set((Array.isArray(stagingManifest?.held) ? stagingManifest.held : []).map(item => sourceIdFromVerdict(item, sources)).filter(Boolean))];
}

function buildStatePromotionManifest(state, stagingManifest, options = {}) {
  const scoped = requireState(state);
  const sources = options.sources || [];
  const sourceIds = assertStatePromotionInput(scoped, stagingManifest, sources);
  ...
  const rows = stagingManifest.rows.map((row, index) => ({
    ...
    currency: 'USD',
    promotion_source: `reviewed-us-${scoped.slug}`
  }));
  const manifest = {
    schema_version: 2,
    type: 'us-reviewed-promotion',
    country_code: 'US',
    region_code: scoped.code,
...
function verifyStatePromotionManifest(state, manifest, stagingManifest, options = {}) {
  const scoped = requireState(state);
  const rebuilt = buildStatePromotionManifest(scoped, stagingManifest, options);
...
function planStateProductionSnapshot(state, snapshot, promotionManifest, stagingManifest, options = {}) {
  const scoped = requireState(state);
  verifyStatePromotionManifest(scoped, promotionManifest, stagingManifest, options);
  ...
  for (const row of existing) {
    ...
    if (identity.id) byId.set(identity.id, row);
  }
  ...
  for (const row of candidates) {
    ...
    if (!identity.source || !identity.application || !identity.id || !eventDate) throw new Error(`${scoped.name} production row missing identity evidence`);

    const idMatch = byId.get(identity.id);
    if (idMatch) {
      const existingIdentity = identityOf(idMatch);
      if (existingIdentity.source === identity.source && existingIdentity.application === identity.application && eventDateOf(idMatch) === eventDate) {
        alreadyPresent.push({ ...row, id: identity.id });
        continue;
      }
      throw new Error(`${scoped.name} production identity collision:${identity.id}`);
    }

    const routeMatches = existing.filter(existingRow => {
      const current = identityOf(existingRow);
      return current.source === identity.source || current.application === identity.application;
    });

    const exactOccurrence = routeMatches.find(existingRow => sameCanonicalRouteAndDate(existingRow, row));
    if (exactOccurrence) {
      const existingIdentity = identityOf(exactOccurrence);
      alreadyPresent.push({ ...row, id: existingIdentity.id, stable_id: existingIdentity.id });
      continue;
    }

    if (routeMatches.some(existingRow => !sameCanonicalRoutes(existingRow, row))) {
      throw new Error(`${scoped.name} production identity collision:${identity.id}`);
    }

    const occurrenceKey = `${identity.source}\u0000${identity.application}\u0000${eventDate}`;
    if (seenIds.has(identity.id) || seenOccurrences.has(occurrenceKey)) throw new Error(`${scoped.name} production duplicate:${identity.id}`);
  ...
  return {
    preview: {
      ...snapshot,
      source: `preview:reviewed-us-${scoped.slug}`,
      total: rows.length,
      rows
    },
    summary: {
...
module.exports = {
  ...
  identityOf,
  eventDateOf,
  sameCanonicalRouteAndDate,
  sourceIdFromRow,
  sourceIdFromVerdict,
  assertStatePromotionInput,
  buildStatePromotionManifest,
  verifyStatePromotionManifest,
  planStateProductionSnapshot
};
