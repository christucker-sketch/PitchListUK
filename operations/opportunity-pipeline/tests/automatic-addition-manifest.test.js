...
test('automatic manifest refreshes exact approved identities and rejects non-ready rows or missing fetch attestation', () => {
  ...
  assert.equal(duplicate.changes.updates.length, 1);
  assert.equal(duplicate.changes.updates[0].row.id, 'opp_new');
  assert.equal(duplicate.changes.updates[0].row.last_checked, '2026-08-21');
