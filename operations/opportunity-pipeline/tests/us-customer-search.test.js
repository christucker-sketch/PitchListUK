...
test('US subscriber search keeps source and application routes', () => {
  ...
  assert.equal(result.total, 1);
  assert.equal(result.rows[0].stable_id, 'opp_us_b');
  assert.match(result.rows[0].application_url, /dallas\/apply/);
});

test('US ZIP radius search uses isolated Texas ZIP resolver and shared haversine maths', () => {
