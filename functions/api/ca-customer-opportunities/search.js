...
const { searchCanadaCustomerRows } = caSearch;

function json(payload, status = 200, headers = {}) {
  return stripeJson(payload, status, headers);
}

function canadaPublicEnabled(env = {}) {
  return ['1', 'true', 'yes'].includes(String(env.FINDPITCHES_CA_PUBLIC_ENABLED || '').trim().toLowerCase());
}

export async function resolveCanadaAccess(request, env, url) {
...
export function parseCanadaSearchOptions(url, fullAccess = false) {
  return {
    fullAccess,
    q: url.searchParams.get('q') || '',
    category: url.searchParams.get('category') || '',
    province: url.searchParams.get('province') || url.searchParams.get('region_code') || '',
...
export async function onRequestGet(context) {
  ...
  if (!canadaPublicEnabled(env)) {
    return new Response('Not found', { status: 404, headers: { 'x-robots-tag': 'noindex' } });
  }

  const url = new URL(request.url);
  const access = await resolveCanadaAccess(request, env, url);
  const result = searchCanadaCustomerRows(
    caOpportunitySnapshot.rows,
    parseCanadaSearchOptions(url, access.mode === 'subscriber')
  );
