...
test('shadow batch only selects validated candidates and promotes ready records',async()=>{
 const store=db({id:'x',market:'GB',region_code:'GB-ENG-KENT',canonical_url:'https://x.test',application_url:'https://x.test/apply',event_name:'Fair',organiser:null,geography_json:'{}',score:80,status:'validated',last_checked:'2026-09-26'});
 const r=await runCustomerPromotionBatch(store,{limit:1});
 assert.equal(r.inspected,1); assert.equal(r.promoted,1);
 assert.ok(store.writes.some(sql=>/INSERT INTO customer_opportunities/.test(sql)));
});
