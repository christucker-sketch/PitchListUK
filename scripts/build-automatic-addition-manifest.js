...
function buildAutomaticAdditionManifest({ snapshot, rows, directReport, reviewedCommit, today, maxAdditions = 3 }) {
  ...
  return {
    ...
    automation: { addition_only: true, source_registry_required: true, direct_fetch_required: true, removals_allowed: false, updates_allowed: false, max_additions: maxAdditions },
...
function main() {
  ...
  const manifest = buildAutomaticAdditionManifest({ snapshot, rows, directReport, reviewedCommit, today: new Date().toISOString().slice(0, 10), maxAdditions: Number(process.env.PITCHLIST_AUTOMATIC_ADDITION_LIMIT || 3) });
  fs.mkdirSync(path.dirname(outputFile), { recursive: true });
  atomicWrite(outputFile, `${JSON.stringify(manifest, null, 2)}\n`);
  console.log(JSON.stringify({ outputFile, beforeCount: snapshot.rows.length, additions: manifest.changes.additions.map(item => ({ event_name: item.row.event_name, source_url: item.row.source_url })), afterCount: snapshot.rows.length + manifest.changes.additions.length }, null, 2));
}
